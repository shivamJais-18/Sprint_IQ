...
export const createBug = async (req, res) => {
  try {
    ...
    await notifyUsers([projectData.manager, validAssignedTo], {
      type: 'Bug Reported',
      title: 'New bug reported',
      message: `"${populatedBug.title}" was reported in ${projectData.name}.`,
      relatedId: bug._id,
...
export const updateBug = async (req, res) => {
  try {
    ...
    const previousAssignedTo = bug.assignedTo ? bug.assignedTo.toString() : null;
    const previousStatus = bug.status;
    const previousSeverity = bug.severity;
    const previousPriority = bug.priority;
    ...
    if (newAssignedTo && newAssignedTo !== previousAssignedTo) {
      await notifyUsers([newAssignedTo], {
        ...
        message: `You were assigned to "${updatedBug.title}".`,
        relatedId: bug._id,
        excludeUserId: req.user._id
      });
    } else if (
      (status !== undefined && status !== previousStatus) ||
      (severity !== undefined && severity !== previousSeverity) ||
      (priority !== undefined && priority !== previousPriority)
    ) {
      await notifyUsers([newAssignedTo, bug.reportedBy], {
        type: 'Bug Updated',
        title: 'Bug updated',
        message: `"${updatedBug.title}" was updated.`,
        relatedId: bug._id,
        excludeUserId: req.user._id
      });
    }
