...
export const createProject = async (req, res) => {
  try {
    ...
    if (normalizedMembers.length) {
      await notifyUsers(normalizedMembers, {
        type: 'Project Member Added',
        title: 'Added to project',
        message: `You were added to ${populatedProject.name} by ${req.user.name || 'the project manager'}.`,
...
export const updateProject = async (req, res) => {
  try {
    ...
    const previousMemberIds = (project.members || []).map((member) => member.toString());
    ...
    if (members !== undefined) {
      ...
      if (newMemberIds.length) {
        await notifyUsers(newMemberIds, {
          type: 'Project Member Added',
          title: 'Added to project',
          message: `You were added to ${updatedProject.name} by ${req.user.name || 'the project manager'}.`,
          relatedId: project._id,
          excludeUserId: req.user._id
        });
      }
    ...
    if (status !== undefined && status !== previousStatus) {
      await notifyUsers([project.manager, ...(project.members || [])], {
        ...
        message: `"${updatedProject.name}" moved to ${status}.`,
        relatedId: project._id,
        excludeUserId: req.user._id
      });
    }
